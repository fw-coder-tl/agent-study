...
import com.tianliang.model.ChatStatus;
import com.tianliang.model.OrderChat;
import com.tianliang.tools.OrderTools;
...
public class PddRefundController {
    ...
    public OrderChat newChat(String userId, String orderId, HttpServletResponse httpServletResponse) {
        ...
        // 模拟数据库创建一个chat的记录，获取他的唯一id
        String chatId = UUID.randomUUID().toString();

        return  chatClient.prompt()
                .user(String.format("我要咨询订单相关的问题，我的用户id是%s，我的订单号是：%s，当前状态是%s",userId,orderId, ChatStatus.CHAT_START.name()))
